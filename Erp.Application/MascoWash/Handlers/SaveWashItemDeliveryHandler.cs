...
{
    internal class SaveWashItemDeliveryHandler
        : IRequestHandler<SaveWashItemDeliveryModel, Result>
    {
        private readonly ISaveDataList _service;

        public SaveWashItemDeliveryHandler(ISaveDataList service)
        {
            _service = service;
        }

        public async Task<Result> Handle(
            SaveWashItemDeliveryModel request,
            CancellationToken cancellationToken)
        {
            ...
            return await _service.SaveWashItemDeliveryData(request);
        }
    }
}
