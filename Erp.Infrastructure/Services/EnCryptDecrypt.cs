...
namespace Erp.Infrastructure.Services
{
    ...
    {
        private readonly string mkhPassword = "mohammadkamrulhasan";
        ...
        {
            var keys = this.DeriveTwoKeys(mkhPassword);

            var plaintext = Encoding.UTF8.GetBytes(message);

            var packed = this.EncryptBytes(keys[0], plaintext);

            return BitConverter.ToString(this.AddMac(keys[1], packed));
        }
        ...
        public String DecryptString(string str)
        {
            ...
            {
                secret[i] = Convert.ToByte(arr[i], 16);
            }
